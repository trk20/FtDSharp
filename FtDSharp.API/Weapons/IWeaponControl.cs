...
public interface IWeaponControl
{
    ...
    /// <summary>
    /// Attempts to fire the weapon(s) using <see cref="FireOptions.Default"/>.
    /// </summary>
    /// <returns>True if any weapon fired successfully.</returns>
    bool Fire();

    /// <summary>
    /// Attempts to fire the weapon(s) with the given options.
    /// </summary>
    /// <param name="options">Failsafe and AI firing policy for this call.</param>
    /// <returns>True if any weapon fired successfully.</returns>
    bool Fire(FireOptions options);

    /// <summary>
    /// Aims at a position and fires if on target using <see cref="FireOptions.Default"/>.
    /// </summary>
    ...
    /// <summary>
    /// Aims at a position and fires if on target with the given options.
    /// </summary>
    /// <param name="worldPosition">The target position to aim at.</param>
    /// <param name="options">Failsafe and AI firing policy for this call.</param>
    /// <returns>True if any weapon fired successfully.</returns>
    bool TryFireAt(Vector3 worldPosition, FireOptions options);
}
